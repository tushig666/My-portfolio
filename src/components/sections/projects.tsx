...
const projectsData = (t: any, language: string) => [
  ...
  },
  {
    ...
    category: language === 'ja' ? "サバイバルホラー" : "Survival Horror",
    image: PlaceHolderImages.find(img => img.id === 'zombie')?.imageUrl || "",
    tags: ["Three.js", "WebAudio", "Interactive"],
    github: "https://github.com/tushig666/zombie-corridor-v1",
  ...
  },
  {
    ...
    category: language === 'ja' ? "モーションデザインAI" : "Motion Design AI",
    image: PlaceHolderImages.find(img => img.id === 'aether')?.imageUrl || "",
  ...
  },
  {
    ...
    category: language === 'ja' ? "AIオーケストレーション" : "AI Orchestration",
    image: PlaceHolderImages.find(img => img.id === 'agents')?.imageUrl || "",
...
export function Projects() {
  ...
  const projects = projectsData(t, language)
    ...
    <section id="projects" className="py-24 bg-black/30">
      <div className="container px-6 mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          <div>
            <div className="text-primary font-bold tracking-widest text-sm mb-4 uppercase">{t.projects.sub}</div>
        ...
        >
          <a href="https://github.com/tushig666" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 text-foreground/50 hover:text-primary font-bold tracking-widest text-sm transition-all group">
            {t.projects.all} <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
          </a>
        </motion.div>
      </div>
    </section>
